...
func NewPowerline(cfg types.Config, cwd string, align constants.Alignment) *types.Powerline {
	p := new(types.Powerline)
	...
	p.Segments = make([][]types.Segment, 1)
	var mods []string
...
func initSegments(p *types.Powerline, mods []string) {
	orderedSegments := map[int][]types.Segment{}
...
func (p *types.Powerline) color(prefix string, code uint8) string {
...
func (p *types.Powerline) fgColor(code uint8) string {
...
func (p *types.Powerline) bgColor(code uint8) string {
	return p.color("48", code)
}

func (p *types.Powerline) appendSegment(origin string, segment types.Segment) {
...
func (p *types.Powerline) newRow() {
	if len(p.Segments[p.curSegment]) > 0 {
		p.Segments = append(p.Segments, make([]types.Segment, 0))
...
func (p *types.Powerline) truncateRow(rowNum int) {
...
func (p *types.Powerline) numEastAsianRunes(segmentContent *string) int {
...
func (p *types.Powerline) drawRow(rowNum int, buffer *bytes.Buffer) {
	row := p.Segments[rowNum]
	numEastAsianRunes := 0
...
func (p *types.Powerline) draw() string {
...
func (p *types.Powerline) hasRightModules() bool {
	return p.rightPowerline != nil && len(p.rightPowerline.Segments[0]) > 0
}

func (p *types.Powerline) supportsRightModules() bool {
	return p.shell.EvalPromptRightPrefix != "" || p.shell.EvalPromptRightSuffix != ""
}

func (p *types.Powerline) isRightPrompt() bool {
