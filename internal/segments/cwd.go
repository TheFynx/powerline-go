...
func maybeAliasPathSegments(p *types.Powerline, pathSegments []pathSegment) []pathSegment {
...
func cwdToPathSegments(p *types.Powerline, cwd string) []pathSegment {
	pathSeparator := string(os.PathSeparator)
...
func maybeShortenName(p *types.Powerline, pathSegment string) string {
...
func escapeVariables(p *types.Powerline, pathSegment string) string {
...
func getColor(p *types.Powerline, pathSegment pathSegment, isLastDir bool) (uint8, uint8, bool) {
...
func segmentCwd(p *types.Powerline) (segments []types.Segment) {
	cwd := p.cwd
	...
	default:
		...
		if p.cfg.CwdMode == "dironly" {
			...
			if maxDepth <= 0 {
				helpers.Warn("Ignoring -cwd-max-depth argument since it's smaller than or equal to 0")
		...
		for idx, pathSegment := range pathSegments {
			...
			if !special {
				if p.align == constants.AlignRight && p.supportsRightModules() && idx != 0 {
					segment.Separator = p.symbols.SeparatorReverseThin
					segment.SeparatorForeground = p.theme.SeparatorFg
				} else if (p.align == constants.AlignLeft || !p.supportsRightModules()) && !isLastDir {
